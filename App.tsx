...
import { Play, Activity, Globe, Lock, Shield, Cpu, RefreshCw, Zap, LayoutTemplate } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const renderSidebar = () => (
    <div className="w-full lg:w-80 bg-slate-950 border-r border-slate-800 flex flex-col h-full z-30">
      <div className="p-6 border-b border-slate-800">
        ...
        <p className="text-xs text-slate-500 font-mono mt-1">v2.0.4 // AUTONOMOUS_AGENT</p>
      </div>
        ...
        {testSuite && (
          <div className="space-y-2">
            ...
            {testSuite.steps.map((step, idx) => (
               <div 
                key={step.id} 
                className={`p-3 rounded-md border text-sm transition-all relative overflow-hidden ${
  ...
  return (
    <div className="flex h-screen w-full bg-[#0b1120] text-slate-200 overflow-hidden selection:bg-blue-500/30">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        {/* Header Area */}
        <header className="h-16 border-b border-slate-800 bg-[#0b1120]/90 backdrop-blur flex items-center justify-between px-6 z-20">
            <div className="flex items-center gap-6 min-w-0">
              <div className={`px-3 py-1 rounded-full text-[10px] font-bold tracking-widest border uppercase flex items-center gap-2 ${
                status === AppStatus.IDLE ? 'bg-slate-900 border-slate-700 text-slate-500' :
                ...
                <div className={`w-1.5 h-1.5 rounded-full ${
                  status === AppStatus.RUNNING ? 'bg-blue-400 animate-pulse' :
              ...
              {config.url && (
                <div className="flex items-center gap-2 text-sm font-mono text-slate-400 min-w-0">
                  <Globe size={14} className="text-slate-600 shrink-0" />
                  <span className="truncate max-w-[200px] md:max-w-[400px] hover:text-slate-300 transition-colors cursor-default" title={config.url}>
                    {config.url}
                  </span>
                </div>
              )}
        ...
        {/* Main Content Area */}
        <main className="flex-1 overflow-hidden relative bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-100">
